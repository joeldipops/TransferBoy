...
int _set_gbRamBank(const GameboyCart gbcart, int bank) {
	if(gbcart.ram!=TRUE) {
        return -1;
    }
...
int _set_gbRomBank(GameboyCart* gbcart, int bank) {
...
GameboyCart initialiseCart(char controllerNumber) {
    ...
    //set bank 0
    if(_set_gbRomBank(&cart, 0x00) != 0) {
        cart.errorCode = -7;
        return cart;
    }

    //get rdata
    if(_get_gbRomAddr(0xC120, data) != 0) {
    ...
    //get rdata
    if(_get_gbRomAddr(0xC140, data) !=0 ) {
...
char importRom(GameboyCart gbcart, ByteArray* romData){
    ...
    for(int bankCount = 0; bankCount < gbcart.rombanks; bankCount++) { //bank count
        ...
        if(_set_gbRomBank(&gbcart, bankCount) != 0) {
            return -1;
        }
...
char importSave(const GameboyCart gbcart, ByteArray* saveData) {
    const int SEGMENT_SIZE = 0x20; // 32
    ...
    for(int bankCount = 0; bankCount < gbcart.rambanks; bankCount++) {
        ...
        for(unsigned long bankOffset = address; bankOffset <= bankSize; bankOffset += SEGMENT_SIZE) { //bank offset
            ...
            if(_get_gbRamAddr(bankOffset, segment) == 0) {
                byte temp[offset];
                memcpy(temp, saveData->Data, offset);
                free(saveData->Data);
                saveData->Data = malloc(offset + SEGMENT_SIZE);
