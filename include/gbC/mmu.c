        ...
        if (lowcation < 0x40) {
            if (lowcation < 0x20) {
                        ...
                        if (lowcation < 0x04) {
                            ...
                            if (lowcation < 0x06) {
                                if (lowcation < 0x05) {
                                    ...
                                    // FF05: Timer
                                    s->TimerCounter = value;
                                }
                            } else {
                                if (lowcation < 0x07) {
                                    // FF06: Timer Modulo
                                    s->TimerResetValue = value;
                                } else {
                                    // FF07: Timer Control
                                    s->TimerControl = value;
                                }
                            }
                        }
                    } else {
                    ...
                    if (lowcation < 0x18) {
                        if (lowcation < 0x14) {
                            if (lowcation < 0x12) {
                                if (lowcation < 0x11) {
                                    // FF10: channel 1 sweep
                                    s->AudioChannel1Sweep = value;
                                } else {
                                    // FF11: channel 1 length/pattern
                                    s->AudioChannel1PatternAndLength = value;
                                }
                            } else {
                                if (lowcation < 0x13) {
                                    // FF12: channel 1 envelope
                                    s->AudioChannel1Envelope = value;
                                } else {
                                    // FF13: channel frequence
                                    s->AudioChannel1FrequencyLow = value;
                                }
                            }
                        } else {
                            if (lowcation < 0x16) {
                                ...
                                if (lowcation < 0x17) {
                                    // FF16: channel 2 length and pattern
                                    s->AudioChannel2PatternAndLength = value;
                                } else {
                                    // FF17: channel 2 envelope
                                    s->AudioChannel2Envelope = value;
                                }
                            }
                        }
                    } else {
                        if (lowcation < 0x1C) {
                            if (lowcation < 0x1A) {
                                if (lowcation < 0x19) {
                                    // FF18: channel 2 frequency
                                    s->AudioChannel2FrequencyLow = value;
                                } else {
                                    // FF19: channel 2 high frequench and control
                                    s->AudioChannel2Flags = value;
                                }
                            } else {
                                if (lowcation < 0x1B) {
                                    // FF1A: channel 3 enable flag
                                    s->AudioChannel3Control = value;
                                } else {
                                    // FF1B: channel 3 length
                                    s->AudioChannel3Length = value;
                                }
                            }
                        } else {
                            if (lowcation < 0x1E) {
                                if (lowcation < 0x1D) {
                                    // FF1C: channel 3 level
                                    s->AudioChannel3Level = value;
                                } else {
                                    // FF1D: channel 3 frequency
                                    s->AudioChannel3FrequencyLow = value;
                                }
                            } else {
                                if (lowcation < 0x1F) {
                                    // FF1E channel 3 frequench high and control
                                    s->AudioChannel3Flags = value;
                                } else {
                                    // FF1F unused channel 4 sweep
                                    ;
                                }
                ...
                if (lowcation < 0x30) {
                    if (lowcation < 0x28) {
                        if (lowcation < 0x24) {
                            if (lowcation < 0x22) {
                                if (lowcation < 0x21) {
                                    // FF20: channel 4 length
                                    s->AudioChannel4Length = value;
                                } else {
                                    // FF21: channel 4 envelope
                                    s->AudioChannel4Envelope = value;
                                }
                            } else {
                                if (lowcation < 0x23) {
                                    // FF22: channel 4 waveform
                                    s->AudioChannel4RNGParameters = value;
                                } else {
                                    // FF23: channel 4 flags
                                    s->AudioChannel4Flags = value;
                                }
                            }
                        } else {
                            if (lowcation < 0x26) {
                                if (lowcation < 0x25) {
                                    // FF24: sound channel control
                                    s->AudioSpeakerControl = value;
                                } else {
                                    // FF25: sound speaker control
                                    s->AudioSpeakerChannels = value;
                                }
                            } else {
                                if (lowcation < 0x27) {
                                    // FF26: Sound master switch
                    ...
                    // FF30 - FF3f: channel 4 wave data
                    s->SoundWaveData[lowcation-0x30] = value;
                }
            }
        } else {
            if (lowcation < 0x60) {
                    ...
                    if (lowcation < 0x48) {
                        if (lowcation < 0x44) {
                            if (lowcation < 0x42) {
                                ...
                                if (lowcation < 0x43) {
                                    // FF42: Background scroll Y
                                    s->BackgroundScrollY = value;
                                } else {
                                    // FF43: Background scroll X
                                    s->BackgroundScrollX = value; 
                                }
                            }
                        } else {
                            if (lowcation < 0x46) {
                        ...
                        if (lowcation < 0x4C) {
                            if (lowcation < 0x4A) {
                                if (lowcation < 0x49) {
                                    // FF48: Sprite Palette 0
                                    s->SpritePalette0 = value;
                                } else {
                                    //FF49: Sprite palette 2
                                    s->SpritePalette1 = value;
                                }
                            } else {
                                if (lowcation < 0x4B) {
                                    // FF4A: Window Y position
                    ...
                    if (lowcation < 0x68) {
                        ...
                        if (lowcation < 0x6C) {
                            if (lowcation < 0x6A) {
                                if (lowcation < 0x69) {
                                    // FF68: Background palette index
                                    s->GbcBackgroundPaletteIndexRegister = value;
                                } else {
                                    // FF69: Background Palette data
                                ...
                                if (lowcation < 0x6B) {
                                    ...
                                    s->GbcSpritePaletteIndexRegister = value;
                                } else {
                                    //FF6B: Sprite palette data
